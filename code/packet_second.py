...
def packet_per_second(data, sampling_interval):
    if not data:
...
def compare(original_file, generated_file, sampling_interval, output_path):
    ...
    time_original, bits_original = packet_per_second(original_traffic, sampling_interval)
    time_generated, bits_generated = packet_per_second(generated_traffic, sampling_interval)
...
def compare_signals_autocorr(original_file, generated_file, sampling_interval, output_path, max_lag=None):
    ...
    _, bits_original = packet_per_second(original_traffic, sampling_interval)
    _, bits_generated = packet_per_second(generated_traffic, sampling_interval)
...
def get_stats(file_path, sampling_interval):
    ...
    _, signal = packet_per_second(traffic, sampling_interval)
    stats = {}
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            "This script provide functions to evaluate packet/second. "
        )
    )
    parser.add_argument(
        "--original", 
    ...
    compare(args.original, args.generated, args.interval, f'{args.output_dir}/packet_per_second.png')

    compare_signals_autocorr(args.original, args.generated, args.interval, f'{args.output_dir}/packet_per_second_autocorr.png')
