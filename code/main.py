...
def create_parser():
    ...
    parser.add_argument(
        ...
        choices=["app-traffic", "clustering", "stats", "distributions", "generate", "ks-comparison", "stats-comparison", "plot"],
    ...
    parser.add_argument(
        "--input", type=str,
        help="Path to the input file (e.g., pcap, CSV, or JSON)."
    )
    parser.add_argument(
        "--output", type=str,
    ...
    parser.add_argument("--ks-generated-traffic", type=str,
                    help="Generated traffic CSV file.")
    #Stats comparison arguments
    parser.add_argument("--original-stats", type=str,
                help="Original traffic statistics file (JSON).")
    parser.add_argument("--generated-stats", type=str,
                    help="Generated traffic statistics file (JSON).")
    return parser
...
def main():
    ...
    elif args.mode == "ks-comparison":
        if not args.ks_original_traffic or not args.ks_generated_traffic:
            raise ValueError("Argument --ks-original-traffic or --ks-generated-traffic is missing.")
        ks.compare_with_ks(args.ks_original_traffic, args.ks_generated_traffic)

    elif args.mode == "stats-comparison":
        if not args.original_stats or not args.generated_stats:
            raise ValueError("Argument --original-stats or --generated-stats is missing.")
        if not args.output:
            raise ValueError("Argument --output is missing.")
        stats_comparison.stats_comparison_xlsx(args.original_stats, args.generated_stats, args.output)
        print(f'Statistics comparison : .xlsx file generated in {args.output}.')
